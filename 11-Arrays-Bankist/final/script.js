...
const calcDisplayBalance = (account) => {
  ...
  elements.labelBalance.textContent = formatCurrency(account.balance);
};
...
const calcDisplaySummary = (account) => {
  ...
  elements.labelSumIn.textContent = formatCurrency(incomes);

  const out = account.movements
    .filter(mov => mov < 0)
    .reduce((acc, mov) => acc + mov, 0);
  elements.labelSumOut.textContent = formatCurrency(Math.abs(out));
  ...
  elements.labelSumInterest.textContent = formatCurrency(interest);
};
...
elements.btnClose.addEventListener('click', (e) => {
  ...
  const username = elements.inputCloseUsername.value;
  const pin = Number(elements.inputClosePin.value);
...
// Previous code remains the same until the login handler...
...
elements.btnLogin.addEventListener('click', (e) => {
  ...
  // Find account without checking pin initially
